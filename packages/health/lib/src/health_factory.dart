...
class HealthFactory {
  ...
  /// Check permissions to access data types in Apple Health or Google Fit.
  ///
  /// Returns true if successful, false otherwise
  ///
  /// Parameters:
  ...
  Future<bool> checkAuthorization(
    ...
    List<String> keys = mTypes.map((e) => _enumToString(e)).toList();
    final bool? isAuthorized = await _channel.invokeMethod('checkAuthorization', {'types': keys, "permissions": mPermissions});
    return isAuthorized ?? false;
  }
